...
LastEditTime: 2022-05-26 21:49:53
Description: None
'''
import datetime
import time
...
driver.get("http://news.tju.edu.cn/")
...
def handle_page(news_list, worksheet, name):
    ...
    for k in range(len(news_list)):
        item = news_list[k]
        print("***********"+item.text+"***********")
        publish_time = item.find_element_by_tag_name("p").text
...
for i in range(len(arr)):
    ...
    news_list = driver.find_element_by_class_name(
        "list_result").find_elements_by_tag_name("ul")[0].find_elements_by_tag_name("li")
    while handle_page(news_list, worksheet, name) is not False:
        print("************NEW TURN STARTS**********")
        next = None
        try:
            next = driver.find_element_by_class_name("p_next")
        except:
            pass
        if next != None:
            next.click()
...
filename = f"天津大学{month_time}各刊物情况.xls"
