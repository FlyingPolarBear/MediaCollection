...
def handle_page(news_list, worksheet, name):
    ...
    for k in range(len(news_list)):
        ...
        verified = name
        if str(year) in publish_time and str(months[-1]) in publish_time:
...
current_time = datetime.datetime.now()
# month_start = current_time.strftime('%Y%m')
month_start = "202203"
month_end = "202206"
filename = f"data/南京大学{month_start}-{month_end}各刊物情况.xlsx"
